...
function loss(d_obs, q0, m0, F)
    local ϕ
    d = F.options.return_array ? vec(d_obs) : d_obs
    g = gradient(Flux.params(q0, m0)) do
        ϕ = misfit_objective(d, q0, m0, F)
        return ϕ
    end
    return ϕ, g[q0], g[m0]
end
    ...
    @testset "Gradient test: $(stype) source" for (stype, F, q) in ginput
        @timeit TIMEROUTPUT "$(stype) source gradient" begin
            ...
            for j=1:maxiter
                ...
                err3[j] = abs(f - f0)
                err4[j] = abs(f - f0 - h*dmgm)
                j == 1 ? prev = 1 : prev = j - 1
                @printf("%5.5e, %5.5e, %5.5e, %5.5e, %5.5e, %5.5e \n", h, h*abs(dmgm), err3[j], err4[j], err3[prev]/err3[j], err4[prev]/err4[j])
                h = h/2f0
            end
